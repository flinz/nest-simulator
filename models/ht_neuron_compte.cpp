...
namespace nest{
  ...
  template <>
  void RecordablesMap<ht_neuron_compte>::create()
  {
 ...
 * ---------------------------------------------------------------- */
  ...
  extern "C"
  inline int ht_neuron_compte_dynamics (double, const double y[], double f[], 
				void* pnode)
  { 
    ...
    // Compte 2000, AMPA has maximal conductance and exponentially decaying gating variable which is increased on spike arrival by 1
  ...
  void nest::ht_neuron_compte::Parameters_::get(DictionaryDatum &d) const
  {
  ...
  void nest::ht_neuron_compte::Parameters_::set(const DictionaryDatum& d)
  {
  ...
  void nest::ht_neuron_compte::State_::get(DictionaryDatum &d) const
  {
    def<double_t>(d, "V_m",     y_[VM]); // Membrane potential
  }

  void nest::ht_neuron_compte::State_::set(const DictionaryDatum& d, 
				   const Parameters_&)  
  {
  ...
  void nest::ht_neuron_compte::init_state_(const Node& proto)
  {
    const ht_neuron_compte& pr = downcast<ht_neuron_compte>(proto);
    S_ = pr.S_;
  }

  void nest::ht_neuron_compte::init_buffers_()
  {
    // Reset spike buffers.
  ...
  void nest::ht_neuron_compte::calibrate()
  {
    B_.logger_.init();  // ensures initialization in case mm connected after Simulate

    V_.cond_steps_.resize(SUP_SPIKE_RECEPTOR-1);

    V_.cond_steps_[AMPA-1] = 1.;  // Compte 2000, AMPA gating variable increases by 1 at spike arrival

    V_.cond_steps_[NMDA-1] = 1.;  // Compte 2000, NMDA gating variable x is increases by 1 at spike arrival

    V_.cond_steps_[GABA-1] = 1.; // Compte 2000, GABA gating variable x is increases by 1 at spike arrival
    
    V_.RefractoryCounts_ = Time(Time::ms(P_.t_ref_)).get_steps();
    assert(V_.RefractoryCounts_ >= 0);
  }

  void nest::ht_neuron_compte::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  void nest::ht_neuron_compte::set_status(const DictionaryDatum &d)
  {
  ...
  void ht_neuron_compte::update(Time const & origin, const long_t from, 
			const long_t to)
  {
  ...
  void nest::ht_neuron_compte::handle(SpikeEvent & e)
  {
  ...
  void nest::ht_neuron_compte::handle(CurrentEvent& e)
  {
    assert(e.get_delay() > 0);
  ...
  void nest::ht_neuron_compte::handle(DataLoggingRequest& e)
  {
    B_.logger_.handle(e);
  }
}
